...
class QAgent:
    def __init__(self, discount_rate = .99, Qtable_dir:str = False): 
        ...
        # Learning rate
        self.learning_rate_range = [0.1, 0.0005] # big fluctuations in environment, use SMALL  learning rate
        self.lr_decay = 300
    ...
    def train(self, dataset:str, simulations:int = 500) -> ndarray:
        ...
        tscv = TimeSeriesSplit(max_train_size=4380)
        ...
        for isim in range(simulations + 1):
            ...
            self.epsilon = interp(isim, [0, self.epsilon_decay], [self.epsilon_start, self.epsilon_end])
            ...
            for i in range(1, 6):
                ...
                counter_split = 0
                terminated = False
                df = f"{i}_fold_train.xlsx"
                env = DataCenterEnv(df)
                tstamps = env.timestamps
                state = preprocess_state(
                    env.reset(), tstamps)
                state_i = self.discretize_state(state)
                ...
                print(f"Fold {i} total reward: ", total_reward)
                val_ = self.test(f"{i}_fold_val.xlsx")
                print(f"Fold {i} validation reward: ", val_)
